...
def displayFrame(name, detections, frame, ani_frame, text, radius1, radius2, radius3, df):
    for detection in detections:
        ...
        anim_results = draw_animation(ani_frame,radius1, radius2, radius3, int(detection.spatialCoordinates.x)/1000,int(detection.spatialCoordinates.z)/1000, METER_TO_PIXEL)
        # last_saved_time = datetime.now()
        if anim_results:
            crossed_radius, x_point, z_point = anim_results
            print(f"Object crossed {crossed_radius} at X: {x_point}, Z: {z_point}")
            print("Object crossed: ", labelMap[detection.label])
            print("Timestamp: ", datetime.now())
            new_data = {'timestamp': datetime.now(), 'Zone': crossed_radius, 'position_x': x_point, 'position_z': z_point, 'label': labelMap[detection.label]}
...
def draw_animation(frame, radius1, radius2, radius3,  x_point, z_point, m_to_pix):
    ...
    crossed_radius = None
    if z_point_pixel < radius1:
        crossed_radius = "Danger Zone"
    elif z_point_pixel < radius2:
        crossed_radius = "Warning Zone"
    elif z_point_pixel < radius3:
        crossed_radius = "Safe Zone"

    if crossed_radius:
        # If a radius is crossed, log the detection
        return crossed_radius, x_point, z_point
